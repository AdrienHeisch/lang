...
pub fn parse<'e, 't> (arena: &'e Arena<Expr<'e>>, tokens:&'t VecDeque<Token>) -> (Vec<&'e Expr<'e>>, Vec<Error>)
{
    ...
    loop
    {
        statements.push(parse_statement(arena, &mut tokens_iter, &mut env, &mut errors, &mut globals));
        if tokens_iter.peek().is_none() { break; }
    }
...
fn parse_expr<'e, 't> (arena: &'e Arena<Expr<'e>>, tokens: &mut TkIter<'t>, env: &mut Environment, errors: &mut Vec<Error>, globals: &mut Vec<Identifier>) -> &'e Expr<'e>
{
    let tk = next(tokens);
    match &tk.def
    {
        ...
        TokenDef::DelimOpen(Delimiter::Pr) => {
            ...
            match tk.def
            {
                ...
                _ => {
                    push_error(errors, format!("Unclosed delimiter : {:?}", Delimiter::Pr), tk.pos);
        ...
        TokenDef::Eof => arena.alloc(Expr { def: ExprDef::End, pos: tk.pos }),
        _ => {
...
fn parse_expr_next<'e, 't> (arena: &'e Arena<Expr<'e>>, tokens: &mut TkIter<'t>, env: &mut Environment, errors: &mut Vec<Error>, globals: &mut Vec<Identifier>, e:&'e Expr<'e>) -> &'e Expr<'e>
{
    let tk = peek(tokens);
    match tk.def
    {
        ...
        TokenDef::DelimOpen(Delimiter::Pr) => {
            next(tokens);
            let (expr_list, tk_delim_close) = make_expr_list(arena, tokens, env, errors, globals, tk);
            arena.alloc(Expr {
        ...
        TokenDef::Dot => {
            next(tokens);
            arena.alloc(Expr { def: ExprDef::Field(e, parse_expr(arena, tokens, env, errors, globals)), pos: tk.pos })
        }
        _ => e
    }
}

fn parse_structure<'e, 't> (arena: &'e Arena<Expr<'e>>, tokens: &mut TkIter<'t>, env: &mut Environment, errors: &mut Vec<Error>, globals: &mut Vec<Identifier>, tk_identifier:&Token) -> &'e Expr<'e>
{
    ...
    match &id
    {
        ...
        b"if\0\0\0\0\0\0" => {
            ...
            arena.alloc(Expr { def: ExprDef::If { cond, then, elze }, pos })
        },
        b"while\0\0\0" => {
            let cond = parse_expr(arena, tokens, env, errors, globals);
        ...
        b"fn\0\0\0\0\0\0" => { //TODO return statement ?
            ...
            if arity > u8::max_value() { push_error(errors, "Too many parameters !".to_owned(), end_tk.pos); }

            let body = parse_expr(arena, tokens, &mut local_env, errors, globals);

            //TODO recursion ?
            declare_local(env, &id, true); //TODO uninitialized var
            arena.alloc(Expr { def: ExprDef::FnDecl { id, params, body }, pos: pos + body.pos })
        },
        b"struct\0\0" => {
            let mut pos = pos;
            let tk = next(tokens);
            ...
            if let Context::TopLevel = env.get_context() {
                ...
                arena.alloc(Expr { def: ExprDef::StructDecl { id, fields }, pos })
            }
        },
        id => {
            if !check_id_exists(env, globals, id) {
                push_error(errors, format!("Unknown identifier : {}", std::str::from_utf8(id).unwrap()), tk_identifier.pos); //FIXME throws an error if function is used before being declared
            ...
            parse_expr_next(arena, tokens, env, errors, globals, arena.alloc(Expr { def: ExprDef::Id(*id), pos }))
        }
    }
}

fn make_binop<'e> (arena: &'e Arena<Expr<'e>>, op:Op, left:&'e Expr<'e>, right:&'e Expr<'e>) -> &'e Expr<'e>
{
    match right.def
    {
        ExprDef::BinOp { op:op_, left:left_, right:right_ } if op.priority() <= op_.priority() =>
                arena.alloc(Expr { def: ExprDef::BinOp { op:op_, left:make_binop(arena, op, left, left_), right:right_ }, pos: left.pos + right.pos }),
        _ =>    arena.alloc(Expr { def: ExprDef::BinOp { op, left, right }, pos: left.pos + right.pos })
    }
}

fn make_ident_list<'t> (tokens: &mut TkIter<'t>, env: &mut Environment, errors: &mut Vec<Error>, tk_delim_open:&Token) -> (Vec<Identifier>, &'t Token)
{
    ...
    loop
    {
        ...
        if let TokenDef::Id(id) = tk_.def {
            next(tokens);
            let id = id;
            declare_local(env, &id, true);
            list.push(id);
        } else {
            push_error(errors, format!("Expected identifier, got : {:?}", tk_.def), tk_.pos);
...
fn declare_local (env: &mut Environment, id:&Identifier, init:bool)
{
    ...
    env.locals[n_locals as usize] = (*id, if init { 1 } else { 0 });
...
{
    arena.alloc(Expr { def: ExprDef::Invalid, pos })
}
