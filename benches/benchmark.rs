...
fn build_ast (c: &mut Criterion) {
    ...
    c.bench_function("build_ast", |b| b.iter(|| lang::build_ast(&program).unwrap()));
}
...
fn compile_bytecode (c: &mut Criterion) {
    let program = std::fs::read_to_string("./code.lang").unwrap();
    let ast = lang::build_ast(&program).unwrap();
    c.bench_function("compile_bytecode", |b| b.iter(|| lang::compile_bytecode(&ast).unwrap()));
}

fn run_bytecode (c: &mut Criterion) {
    let program = std::fs::read_to_string("./code.lang").unwrap();
    let ast = lang::build_ast(&program).unwrap();
    let bytecode = lang::compile_bytecode(&ast).unwrap();
    c.bench_function("run_bytecode", |b| b.iter(|| lang::run_bytecode(&bytecode).unwrap()));
}
